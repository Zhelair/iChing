...
export function OpenAttentionPractice({ copy, detail, locale }: { copy: DaoCopy; detail: DaoPracticeDetailCopy; locale: Locale }) {
  ...
  return <section className="surface dao-practice dao-open-attention" aria-labelledby="open-attention-practice-title">
    <div className="dao-practice__copy"><p className="eyebrow">{copy.practice}</p><h2 id="open-attention-practice-title" className="mt-3 text-3xl sm:text-5xl">{detail.howToPractice}</h2><p className="mt-4 max-w-xl leading-7 text-[var(--ink-soft)]">{detail.openIntro}</p><p className="dao-safety mt-5">{detail.openSafety}</p></div>
    <div className="dao-practice__player">
      ...
      <div className="dao-practice__scene dao-open-attention__scene" aria-hidden="true"><svg viewBox="0 0 420 250"><circle cx="210" cy="125" r={36 + progress * 70} /><circle cx="210" cy="125" r={70 + progress * 55} /><circle cx="210" cy="125" r={104 + progress * 38} /><path d="M34 178C98 138 145 194 212 153C279 112 329 161 386 127" /></svg><div><span>{state === 'complete' ? copy.complete : detail.openStages[stage]}</span><strong>{state === 'idle' ? `${duration / 60} ${copy.minutes}` : time}</strong></div></div>
